...
uint32_t GetLatency(rtff::Filter& filter) {
  ...
  for (uint32_t sample_idx = 0; sample_idx < content.size() - block_size; sample_idx += block_size) {
    float* sample_ptr = content.data() + sample_idx;
  ...
  Eigen::Map<const Eigen::Matrix<float, Eigen::Dynamic, 1>>(content.data(), content.size()).maxCoeff(&max_index);
