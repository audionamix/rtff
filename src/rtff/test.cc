...
class MyFilter : public rtff::Filter {
 private:
  void ProcessFreqBlock(rtff::FrequentialBuffer* buffer) override {
    for (uint8_t channel_idx = 0; channel_idx < buffer->channel_count(); channel_idx++) {
...
TEST(RTFF, Basis) {
  ...
  for (uint32_t sample_idx = 0;
    ...
    memcpy(buffer.data(), sample_ptr, block_size * channel_number * sizeof(float));
    
    filter.ProcessBlock(&buffer);
    
    memcpy(sample_ptr, buffer.data(), block_size * channel_number * sizeof(float));
